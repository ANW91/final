...
void loop() {
  ...
  delay(100);
  //lsm.read();
  post(latitude, longitude, speed_kph, volt, accelx, accely, accelz);
...
void post(float lat, float lon, float speed_kph, uint16_t volt, int accelx, int accely, int accelz){
   // Post data to website
        uint16_t statuscode;
        int16_t length;
        char url[80] = "http://dmgilmour.herokuapp.com/data";
        char data[80], latS[12], lonS[12];
        String latT, lonT, voltT, idT, movingT, temp;
